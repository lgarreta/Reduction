...
HOME   <<- Sys.getenv ("PFOLDRED_HOME")
if (HOME=="") {
	message ("Error: Environment variable PFOLDRED is empty!! Execute installer.")
	quit ()
}
dyn.load (paste0(HOME,"/tmscorelg.so"))
message ("Using tmscore library from: ", HOME)
...
readCheckParameters <- function (args) {
	...
	args$outDir = paste0 ("out-", basename (args$inDir))
