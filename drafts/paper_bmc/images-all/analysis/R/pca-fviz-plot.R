...
main <- function () {
	...
	name = basename (strsplit (clusterFile, "[.]")[[1]][1])
	pdfFile          = paste (name, "-groups.pdf", sep="")
	pdfFileDendogram = paste (name, "-dendogram.pdf", sep="")
	...
	msg ("Loading hc data... ")
	load ("pca-villinfhb-reduced7k.hc")
	...
	msg ("Plotting hcut dendogram...")
		fviz_dend(as.hclust(hc), show_labels = FALSE, rect = TRUE)
